...
Line Lore는 _"이 코드가 왜 여기 있지?"_ 라는 질문에 3초 만에 답합니다. 커서를 라인에 놓고, git blame을 통해 추적하여, 해당 코드를 도입한 PR을 확인하세요 — VSCode 안에서 모든 것이 완료됩니다.

## 설치

Line Lore는 아직 VSCode Marketplace에 게시되지 않았습니다. `.vsix` 파일로 수동 설치하세요:

1. [GitHub Releases](https://github.com/vincent-kk/line-lore-vscode/releases)에서 최신 `.vsix` 파일을 다운로드합니다
2. VSCode에서 설치합니다:
   - **커맨드 팔레트**: `Cmd+Shift+P` (macOS) / `Ctrl+Shift+P` (Windows/Linux) → `Extensions: Install from VSIX...` → 다운로드한 파일 선택
   - **CLI**: `code --install-extension line-lore-<version>.vsix`
3. 안내에 따라 VSCode를 다시 로드합니다
...
- **인라인 데코레이션**: 추적된 라인 옆에 `← PR #42` 고스트 텍스트 표시
...
| 명령어 | 설명 |
| --- | --- |
| `Line Lore: Trace PR` | 현재 라인의 원본 PR 추적 |
| `Line Lore: Trace PR (Range)` | 선택한 라인 범위 추적 |
| `Line Lore: Explore Graph` | PR → 이슈 그래프 탐색 |
| `Line Lore: Health Check` | 운영 레벨 및 플랫폼 상태 확인 |
| `Line Lore: Clear Cache` | 캐시된 추적 데이터 삭제 |

## 설정

| 설정 | 기본값 | 설명 |
| --- | --- | --- |
| `lineLore.enabled` | `true` | 확장 프로그램 활성화/비활성화 |
| `lineLore.hoverProvider.enabled` | `true` | 호버 툴팁에 추적 정보 표시 |
| `lineLore.inlineDecoration.enabled` | `true` | 추적 후 인라인 PR 번호 표시 |
| `lineLore.inlineDecoration.timeout` | `30` | 데코레이션 자동 제거 시간 (초, 0 = 제거 안 함) |
| `lineLore.trace.deep` | `false` | squash 머지를 위한 딥 추적 활성화 |
| `lineLore.trace.noAst` | `false` | AST 분석 비활성화 |
| `lineLore.trace.noCache` | `false` | 캐싱 비활성화 |
