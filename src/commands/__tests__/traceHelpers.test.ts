...
const mockResult: TraceFullResult = {
  nodes: [{ type: 'pull_request', trackingMethod: 'api', confidence: 'exact', prNumber: 42, prUrl: 'https://github.com/test' }],
  operatingLevel: 2,
...
describe('showTraceResult', () => {
  ...
  it('shows Open PR and Copy Link buttons when no detailPanel', async () => {
    const display: DisplayResult = { found: true, prNumber: 42, prTitle: 'test', prUrl: 'https://github.com/test', operatingLevel: 2, warnings: [] };

    await showTraceResult(display, mockResult, '/file.ts', 1, 'No PR.');

    expect(vscode.window.showInformationMessage).toHaveBeenCalledWith('PR #42: test', 'Open PR', 'Copy Link');
  });
  ...
  it('shows Show Details button when detailPanel provided', async () => {
    ...
    await showTraceResult(display, mockResult, '/file.ts', 1, 'No PR.', mockPanel as never);

    expect(vscode.window.showInformationMessage).toHaveBeenCalledWith('PR #42: test', 'Open PR', 'Copy Link', 'Show Details');
  });

  it('calls detailPanel.show on Show Details click', async () => {
    const display: DisplayResult = { found: true, prNumber: 42, prTitle: 'test', prUrl: 'https://github.com/test', operatingLevel: 2, warnings: [] };
    const mockPanel = { show: vi.fn(), dispose: vi.fn() };
    vi.mocked(vscode.window.showInformationMessage).mockResolvedValue('Show Details' as never);

    await showTraceResult(display, mockResult, '/file.ts', 10, 'No PR.', mockPanel as never, 20);

    expect(mockPanel.show).toHaveBeenCalledWith('/file.ts', 10, mockResult, 20);
  });

  it('shows warning when PR not found', async () => {
    const display: DisplayResult = { found: false, commitSha: 'abc123', operatingLevel: 0, warnings: [] };

    await showTraceResult(display, mockResult, '/file.ts', 1, 'No PR found.');

    expect(vscode.window.showWarningMessage).toHaveBeenCalledWith('No PR found. Commit: abc123');
  });
});

describe('handleTraceError', () => {
  const mockStatusBar = { showError: vi.fn(), showLoading: vi.fn(), showResult: vi.fn() };
  ...
  it('shows warning message for LineLoreError with warning severity', () => {
    ...
    expect(vscode.window.showWarningMessage).toHaveBeenCalledWith(expect.stringContaining('GitHub CLI not authenticated'));
  });

  it('shows generic error for non-LineLoreError', () => {
    handleTraceError(new Error('random'), mockStatusBar as never);

    expect(mockStatusBar.showError).toHaveBeenCalledWith('An unexpected error occurred.');
    expect(vscode.window.showErrorMessage).toHaveBeenCalledWith('An unexpected error occurred.');
  });
});
